...
Database::load()
{
	...
	pFile = fopen (_internal_path.str().c_str(), "rb" );
	if(pFile==NULL) 
	{
	...
	result = fread(this->internal_tag_arr, 1, this->entity_num, pFile);
	if(result != this->entity_num) 
	{
	...
	//cout << "finish load internal vertices." << endl;
...
Database::setInternalVertices(const char* _path){
	...
	this->internal_tag_arr = new char[this->entity_num + 1];
	memset(this->internal_tag_arr, '0', this->entity_num);
	this->internal_tag_arr[this->entity_num] = 0;
	...
	while(getline(infile, buff)){
		...
		this->internal_tag_arr[_entity_id] = '1';
	...
	stringstream _internal_path;
	_internal_path << this->getStorePath() << "/internal_nodes.dat";
	cout << "import internal vertices to database " << _internal_path.str() << " done." << endl;
	
	FILE * pFile;
	pFile = fopen (_internal_path.str().c_str(), "wb");
	fwrite(this->internal_tag_arr, sizeof(char), this->entity_num, pFile);
	fclose(pFile);
    ...
    for ( ; itr != _result_list.end(); itr++)
    {
		...
		*/
        ...
        for (int i = 0; i < id_list_len; i++)
        {
			...
			if ('0' == this->internal_tag_arr[id_list[i]])
			{
				int* result = new int[_var_num + 1];
				memcpy(result, itr_result,
					   sizeof(int) * (_var_num + 1));
...
bool Database::select(vector<int*>& _result_list,int _var_id,int _pre_id,int _var_id2,const char _edge_type,int _var_num)
{
    ...
    for ( ;	itr != _result_list.end(); itr++)
    {
		...
		if ('0' == this->internal_tag_arr[itr_result[_var_id]] && '0' == this->internal_tag_arr[itr_result[_var_id2]])
		{
			continue;
		}
...
Database::join(SPARQLquery& _sparql_query)
{
    ...
    for(int i=0; i < basic_query_num; i++)
    {
        ...
        this->join_pe(basic_query);
        long after_joinbasic = Util::get_cur_time();
        //cout << "after join_pe : used " << (after_joinbasic-after_add_literal) << " ms" << endl;
        ...
        //basic_query->dupRemoval();
		printf("There are %d partial results in Client %s!\n", (basic_query->getResultList()).size(), this->getDBInfoFile().c_str());
    }
    return true;
}
...
bool
Database::join_pe(BasicQuery* basic_query)
{
	...
	for(int res_j = 0; res_j < var_num; res_j++){
		...
		dealed_subquery.insert(this->printDealedVertices(dealed_id_list, var_num));
		/*
			...
			*/
			...
			for (int i = 0; i < var_degree; i++)
			{
				...
				if (!dealed_id_list[var_id2])
				{
					...
					string _tmp_subquery_str = this->printDealedVertices(dealed_id_list, var_num);
					/*
					cout << _tmp_subquery_str << "======";
					for(set<string>::iterator it = dealed_subquery.begin(); it != dealed_subquery.end(); it++){
						cout << *it << ";";
					}
					cout << endl;
					*/
					bool no_false_tag = true;
					for(int _tmp_i = 0; _tmp_i < var_num; _tmp_i++){
						no_false_tag = no_false_tag & dealed_id_list[_tmp_i];
					}
					
					if(dealed_subquery.count(_tmp_subquery_str) != 0 && !no_false_tag){
						break;
					}else{
						dealed_subquery.insert(_tmp_subquery_str);
					}					
					join_tag = true;
					dealed_id_list[var_id2] = true;
	...
	//this->printResList(all_result_list, var_num + 1,  "\t");

...
{
#ifdef MULTI_INDEX
	SLOG_CORE("IN GeneralEvaluation::handle");
	...
	if ((**iter).getEncodeBasicQueryResult())
	{
		...
		case 6:
			...
			break;			
		default:
			SLOG_CORE("not support this method");

		}
		if(this->method == 6)
		{
			SLOG_CORE("BasicQuery -- Final result size: " << (*iter)->getResultList()[0][0]);
			(*iter)->getResultList().clear();
		}
		else
		{
			SLOG_CORE("BasicQuery -- Final result size: " << (*iter)->getResultList().size());
		}
	}
#else
	...
	TYPE_TRIPLE_NUM* pre2sub, TYPE_TRIPLE_NUM* pre2obj, bool * dealed_triple)
{
	...
	SLOG_CORE("start constant filter here \n");
	for (int _var_i = 0; _var_i < var_num; _var_i++)
	{
		...
		SLOG_CORE("\tVar" << _var_i << " " << basic_query->getVarName(_var_i));
	   // this->basic_query->setReady(_var_i);
            ...
            if (_list.size() > 0)
	    {
			...
			SLOG_CORE("\t\t[" << _var_i << "] after constant filter, candidate size = " << _list.size() << "\n\n");
	}

	SLOG_CORE("pre filter start here");
	...
	for(int _var = 0; _var < var_num; _var++)
	{
		...
		SLOG_CORE("\tVar" << _var << " " << basic_query->getVarName(_var));
	    ...
	    */
	    ...
	    for (int i = 0; i < var_degree; i++)
	    {
	        ...
	        {
				SLOG_CORE("skip the prefilter because the pre var is not efficent enough");
	        	continue;
	        }

	        int prefilter_num;
		if((edge_type == GlobalTypedef::EDGE_OUT && (prefilter_num = pre2obj[pre_id]) > estimate_num[_var]) ||
			(edge_type == GlobalTypedef::EDGE_IN && (prefilter_num = pre2sub[pre_id]) > estimate_num[_var]))
		{
			SLOG_CORE("skip the prefilter because the constant filter is strong enough");
  ...
  //          cout << "after in_edge_filter, the cans size = 0" << endl;
        ...
        for(it = out_edge_pre_id.begin(); it != out_edge_pre_id.end(); ++it)
	{
		...
		SLOG_CORE("\t\t[" << _var << "] after pre var filter, candidate size = " << cans.size() << "\n\n");
	}
...
{
	SLOG_CORE("Special Case: consider pre var in this triple");
	...
	if (total_num == 2)
	{
		SLOG_CORE("Special Case 1");
		int svpos = _bq->getSelectedVarPosition(triple.subject);
		int ovpos = _bq->getSelectedVarPosition(triple.object);
		SLOG_CORE("subject: "<<triple.subject<<" "<<svpos);
		SLOG_CORE("object: "<<triple.object<<" "<<ovpos);
		SLOG_CORE("predicate: "<<triple.predicate<<" "<<pvpos);
...
{
	SLOG_CORE("Special Case: consider constant triple");
	Triple triple = _bq->getTriple(0);
	_result_list.clear();
...
{
  SLOG_CORE("Special Case:select * and write to stream");
  // int varNum = _bq->getVarNum();
  //all variables(not including pre vars)
  // int total_num = _bq->getTotalVarNum();
  // int pre_varNum = _bq->getPreVarNum();

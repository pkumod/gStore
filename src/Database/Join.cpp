...
Join::select()
{
	...
	if (maxi == -1)
	{
		SLOG_ERROR("error to select the first one to join");
	}
	else
	{
		this->start_id = maxi;
...
Join::copyToResult()
{
	...
	{
		SLOG_ERROR("terrible error in copyToResult!");
		return;
	}
...
Join::join_two(vector< vector<int> >& _edges, IDList& _can_list, unsigned _can_list_size, int _id, bool _is_ready)
{
	...
	for(TableIterator it0 = this->current_table.begin(); it0 != this->new_start;)
	{
		...
		{
			std::string code_print = "now the new_start is:";

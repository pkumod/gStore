...
void TempResult::doJoin(TempResult &x, TempResult &r)
{
	...
	// large_begin = gs::TimeUtil::timestamp();
	// small_begin = gs::TimeUtil::timestamp();
	...
	// small_end = gs::TimeUtil::timestamp();
	...
	// large_end = gs::TimeUtil::timestamp();
	// if (common.empty())
...
void TempResult::print(int no)
{
	...
	if (no == -1)
		SLOG_CORE("temp result:");
	else
		SLOG_CORE("temp result no." << no << ":");

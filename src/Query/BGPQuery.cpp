...
bool BGPQuery::build_edge_info(KVstore *_kvstore) {
	...
	for(unsigned i = 0; i < this->triple_vt.size(); ++i){
		...
		// deal with s_var
		if(s_is_var)
			this->var_vector[id_position_map[s_id]]->update_so_var_edge_info(o_id,p_id,GlobalTypedef::EDGE_OUT,i,p_is_var,o_is_var);

		// deal with o_var
		if(o_is_var)
			this->var_vector[id_position_map[o_id]]->update_so_var_edge_info(s_id,p_id,GlobalTypedef::EDGE_IN,i,p_is_var,s_is_var);

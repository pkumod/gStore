...
{
	latch.lockExclusive();
	std::thread entity([this, &entiys, &kv_store](){ this->changeEntiy(entiys, kv_store); });
	std::thread literal([this, &literals, &kv_store](){ this->changeLiteral(literals, kv_store); });
	std::thread predicate ([this, &pricates, &kv_store](){ this->changePrivates(pricates, kv_store); });
	entity.join();
	literal.join();
	predicate.join();
	latch.unlock();

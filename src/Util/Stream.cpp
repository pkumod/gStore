...
Stream::outputCache()
{
	...
	std::stable_sort(this->tempst.begin(), this->tempst.end(), mycmp);
    #else
...
Stream::setEnd()
{
    ...
    if(this->inMem)
    {
		...
		if(this->needSort)
		{
			//DEBUG2
            #ifndef PARALLEL_SORT
			std::stable_sort(this->ansMem.get(), this->ansMem.get() + this->rownum, mycmp);
            #else
			omp_set_num_threads(thread_num);
			__gnu_parallel::stable_sort(this->ansMem.get(), this->ansMem.get() + this->rownum, mycmp);
            #endif
		}
		return;
    }

...
namespace cluster
{
    ...
    bool ClusterEntityLeader::waitTimerPassNum(std::string db_name, uint64 index, ClusterOperation operation, uint64 end_time)
    {
        ...
        while (1)
        {
            uint64 current_time = gs::TimeUtil::timestamp();
            ...
            if (pass_num >= need_num)
                break;
            if (gs::TimeUtil::timestamp() >= end_time)
                break;
        }
    ...
    uint64 ClusterEntityLeader::getTimeOutEndTime(const std::string& db_name, const std::string& file_name)
    {
        ...
        {
            return gs::TimeUtil::timestamp() + getAppendTimeout(db_name, file_name);
        }
        
        return gs::TimeUtil::timestamp() + std::atoi(Util::getConfigureValue("cluster_relpy_timeout").c_str())*1000;
    }

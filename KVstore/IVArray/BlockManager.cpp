/*=======================================================================
 * File name: BlockManager.cpp
 * Author: Zongyue Qin
 * Mail: qinzongyue@pku.edu.cn
 * Last Modified: 2018-02-08
 * Description:
 * Implementation of class BlockManager
 * ======================================================================*/
...
BlockManager::BlockManager(string& _filename, string& _mode, unsigned _keynum )
{
	cout << "Initialize BlockManager...";
	//dir_path = _dir_path;
	...
	{
		cout << "Error in BlockManager: Open error" << endl;
		return;
	}
	...
	else // open mode
	{
		...
		unsigned tmp_index, tmp_len, offset = sizeof(unsigned);
...
void
BlockManager::SaveFreeBlockList()
{
...
bool
BlockManager::ReadValue(unsigned _blk_index, char *&_str, unsigned &_len)
{
	if (_blk_index <= 0)
	{
...
//	fseek(ValueFile, BLOCK_SIZE * (_blk_index - 1) + sizeof(unsigned), SEEK_SET);
	unsigned offset = BLOCK_SIZE * (_blk_index - 1) + sizeof(unsigned);
	...
	if (_str == NULL)
	{
		cout << "BlockManager Error: fail when new" << endl;
		return false;
	}
	...
	do
	{
		...
		offset = BLOCK_SIZE * (next_blk - 1);
		pread(fd, &next_blk, 1 * sizeof(unsigned), offset);
		//fseek(ValueFile, BLOCK_SIZE * (next_blk - 1), SEEK_SET);
...
bool
BlockManager::AllocBlock(unsigned len)
{
...
bool
BlockManager::getWhereToWrite(unsigned _len)
{
...
unsigned
BlockManager::WriteValue(const char *_str, const unsigned &_len)
{
	...
	while (p != NULL)
	{
		...
		unsigned offset = (BLOCK_SIZE) * (p->num - 1);
...
bool 
BlockManager::FreeBlocks(const unsigned index)
{
	...
	while (_index > 0)
	{
		unsigned offset = (_index - 1) * BLOCK_SIZE;

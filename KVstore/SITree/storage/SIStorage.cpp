 ...
 */
bool SIStorage::PreRead(SINode *&_root, SINode *&_leaves_head, SINode *&_leaves_tail)
{
 ...
 */
bool SIStorage::fullLoad(SINode *&_root)
{
    if (_root == NULL)
    {
        return false;
 ...
 */
long SIStorage::Address(unsigned block_num) const // BETTER: inline function
{
    if (block_num == 0)
        return 0;
 ...
 */
void SIStorage::FreeBlock(unsigned block_num)
{
 ...
 */
void SIStorage::ReadAlign(unsigned *_next)
{
 ...
 */
void SIStorage::WriteAlign(unsigned *_curnum, bool &_SpecialBlock)
{
    if (ftell(tree_fp_) % BLOCK_SIZE == 0)
    {
 ...
 */
bool SIStorage::ReadNode(SINode *_np, long long *_request)
{
 ...
 * @return if success
 */
bool SIStorage::CreateNode(SINode *&_np)
{
 ...
 */
bool SIStorage::WriteNode(SINode *_np)
{
 ...
 */
bool SIStorage::ReadBstr(Bstr *_bp, unsigned *_next)
{
    // long address;
    unsigned len, i, j;
 ...
 */
bool SIStorage::writeBstr(const Bstr *_bp, unsigned *_curnum, bool &_SpecialBlock)
{
 ...
 */
bool SIStorage::WriteTree(SINode *_np) //
{
 ...
 */
void SIStorage::updateHeap(SINode *_np, unsigned _rank, bool _inheap) const
{
 ...
 */
bool SIStorage::request(long long needed_mem) // aligned to byte
 ...
 */
bool SIStorage::handler(unsigned long long needed_mem) //>0
{
    SINode *p = nullptr;

...
- 更新功能
  ...
  - **重构bin目录结构**：只保留gserver服务和gconsole两种操作模式；

  - **新增操作任务管理**：支持对执行中的查询任务进行终止操作；

  - **重构内存管理机制**：采用按需申请内存块机制，减少内存拷贝的环节，批量新增的效率提升1倍以上；

  - **重构自定义函数模块**：新增内置函数pfn.fun_name, 支持select调用；

  - **新增static gcc安装包**：支持使用自定义函数；

  - **新增rpm和deb包**：支持rpm和dpkg命令安装gstore；

  - **优化数据更新逻辑**：采用多线程处理id2string和string2id的映射，以及StringIndex的重建，更新效率提升1倍；

  - **优化数据库构建逻辑**：采用并行处理机制，效率提升30%；

  - **优化子查询doJoin操作**：使用缓存数据操作，减少I/O；

  - **新增内置高级函数**：新增三个高级函数，分别是：所有最短路径（shortestPaths），所有K跳邻居（kHopAllNeighbors）、文本相似度（SIMILARITY）；

  - **新增若干API接口**：如加载CSR资源（loadcsr)、获取Schema信息（schema）等；

  - **优化API接口**：构建、加载、卸载、备份、还原等接口支持异步模式；

  - **支持生成schema**：支持构建接口和批量插入数据接口，构建生成schema信息；

  - **支持事务内存可控加载**：支持通过传递参数，选择加载事务初始化内存；

  - **Bug修复**：修复了一系列Bug，如频繁增删同一批数据引起磁盘空间不断膨胀的问题。
...
- 更新功能
  ...
  - **新增Aggregate函数SAMPLE**：新增SAMPLE函数，可以从包含多个值的集合中随机返回其中一个值，并支持在SELECT中调用；

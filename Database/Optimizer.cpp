...
tuple<bool,IntermediateResult> Optimizer::ExecutionTopK(shared_ptr<BGPQuery> bgp_query, shared_ptr<TopKSearchPlan> &tree_search_plan,
  ...
  {
    ss << "pos["<<pos_id_pair.first<<"]"<<"|"<<pos_id_pair.second<<"| "<<bgp_query->get_var_name_by_id(pos_id_pair.second)<<"\t";
  }
  SLOG_CORE(ss.str());
#endif
  ...
  for(int i =1; i<=query_info.limit_num_ + deleted_num; i++)
  {
    ...
    stringstream ss;
      ...
      ss << "get top-" << i << " "<<root_fr->pool_[i-1].cost;
    }
#endif
  ...
  for(decltype(result_list->size()) i =0;i<result_list->size();i++)
  {
    auto& rec = *it;
    stringstream ss;
    ss << " record["<<i<<"]"<<" score:"<<score_list[i];
    for(unsigned j =0;j<var_num;j++)
      if(is_predicate_var[(*pos_var_mapping)[j]])
        ss << " "<<kv_store_->getPredicateByID(rec[j])<<"["<<rec[j]<<"]";
      else
        ss <<" "<<kv_store_->getStringByID(rec[j])<<"["<<rec[j]<<"]";
    SLOG_CORE(ss.str());
    it++;
  }
#endif
...
void Optimizer::PrintTable(IntermediateResult& result,
  ...
  SLOG_CORE("\tresult size = " << result.values_->size());

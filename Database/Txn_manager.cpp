...
bool Txn_manager::undo(string str, txn_id_t TID)
{
	...
	if (str[0] == 'I') {
		...
		SLOG_CORE("wrong undo sparql: " << str);
		return false;
	}
...
bool Txn_manager::redo(string str, txn_id_t TID)
{
	...
	if (str[0] == 'I') {
		...
		SLOG_CORE("wrong redo sparql: " << str);
		return false;
	}
...
int Txn_manager::Commit(txn_id_t TID)
{
	...
	shared_ptr<Transaction> txn = get_transaction(TID);
	if (txn == nullptr) {
		SLOG_CORE("wrong transaction id!");
		// checkpoint_lock.unlock();
		return -1;
	}
	else if (txn->GetState() != TransactionState::RUNNING) {
		SLOG_CORE("transaction not in running state! commit failed" << " " << (int)txn->GetState());	
		// checkpoint_lock.unlock();
		return 1;
	}
...
int Txn_manager::Rollback(txn_id_t TID)
{
	...
	if (txn == nullptr) {
		...
		SLOG_CORE("transaction not in running state! rollback failed");
		return 1;
	}
	return Abort(TID);
}
...
int Txn_manager::Query(txn_id_t TID, string sparql, string& results)
{
	...
	if (txn->GetState() != TransactionState::RUNNING) {
		SLOG_CORE("transaction not in running state! Query failed" <<  " " << (int)txn->GetState());
		return -99;
	}
	int ret_val;
	ResultSet rs;
...
void Txn_manager::Checkpoint()
{
	...
	//SLOG_CORE("sub_ids.size(): " << sub_ids.size() << endl;
	//SLOG_CORE("pre_ids.size(): " << pre_ids.size() << endl;
	//SLOG_CORE("obj_ids.size(): " << obj_ids.size() << endl;
	//SLOG_CORE("obj_literal_ids.size(): " << obj_literal_ids.size() << endl;
 	if(db != nullptr)

...
Statistics::Statistics(const string &path, int limitID_pre) {
    ...
    this->one_edge_type_num = 0;
    this->one_edge_type_pre_num = 0;

    this->two_edges_type1_num = 0;
    this->two_edges_type1_pre_num = 0;
    this->two_edges_type2_num = 0;
    this->two_edges_type2_pre_num = 0;
    this->two_edges_type3_num = 0;
    this->two_edges_type3_pre_num = 0;
}
...
//                        这是这个函数的主体
                        ...
                        pair<unordered_multimap<TYPE_ENTITY_LITERAL_ID, TYPE_ENTITY_LITERAL_ID>::iterator, unordered_multimap<TYPE_ENTITY_LITERAL_ID, TYPE_ENTITY_LITERAL_ID>::iterator> s1_range = entityid_to_typeid_map.equal_range(p1_so_id_list[i]);
                        pair<unordered_multimap<TYPE_ENTITY_LITERAL_ID, TYPE_ENTITY_LITERAL_ID>::iterator, unordered_multimap<TYPE_ENTITY_LITERAL_ID, TYPE_ENTITY_LITERAL_ID>::iterator> s2_range = entityid_to_typeid_map.equal_range(p1_so_id_list[i+1]);
                        pair<unordered_multimap<TYPE_ENTITY_LITERAL_ID, TYPE_ENTITY_LITERAL_ID>::iterator, unordered_multimap<TYPE_ENTITY_LITERAL_ID, TYPE_ENTITY_LITERAL_ID>::iterator> s3_range = entityid_to_typeid_map.equal_range(po_list[j+1]);
                        ...
                        for(auto s1 = s1_range.first; s1 != s1_range.second; ++s1){
                            for(auto s2 = s2_range.first; s2 != s2_range.second; ++s2){
                                for(auto s3 = s3_range.first; s3 != s3_range.second; ++s3){
                                    ...
                                    if(two_edges_type1id_to_num.find(now_two_pre_id) == two_edges_type1id_to_num.end()){

                                        two_edges_type1_pre_num += 1;
                                        two_edges_type1_num += 1;

//                                        cout << kv_store->getEntityByID(now_two_edges_entity_type_id.s1_type_id) << "  " << kv_store->getPredicateByID(pre_id) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << "  " << kv_store->getPredicateByID(po_list[j]) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << endl;
                                        ...
                                        two_edges_type1id_to_num.insert(make_pair(now_two_pre_id, this_type_num));
                                        ...
                                        if(two_edges_type1id_to_num[now_two_pre_id].find(now_two_edges_entity_type_id) != two_edges_type1id_to_num[now_two_pre_id].end()){
//                                            如果有这样的 two_pre_id
...
//                                            cout << kv_store->getEntityByID(now_two_edges_entity_type_id.s1_type_id) << "  " << kv_store->getPredicateByID(pre_id) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << "  " << kv_store->getPredicateByID(po_list[j]) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << endl;
                                            ...
                                            two_edges_type1id_to_num[now_two_pre_id].insert(make_pair(now_two_edges_entity_type_id, 1));
                        ...
                        for(auto s1 = s1_range.first; s1 != s1_range.second; ++s1){
                            for(auto s2 = s2_range.first; s2 != s2_range.second; ++s2){
                                for(auto s3 = s3_range.first; s3 != s3_range.second; ++s3){
                                    ...
                                    if(two_edges_type2id_to_num.find(now_two_pre_id) == two_edges_type2id_to_num.end()){
...
//                                        cout << kv_store->getEntityByID(now_two_edges_entity_type_id.s1_type_id) << "  " << kv_store->getPredicateByID(pre_id) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << "  " << kv_store->getPredicateByID(po_list[j]) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << endl;
                                        ...
                                        unordered_map<TWO_EDGES_ENTITY_TYPE_ID, int, TWO_EDGES_ENTITY_TYPE_ID::TWO_EDGES_ENTITY_TYPE_ID_Hash> this_type_num;
                                        this_type_num.insert(make_pair(now_two_edges_entity_type_id, 1));
                                        two_edges_type2id_to_num.insert(make_pair(now_two_pre_id, this_type_num));
                                        ...
                                        if(two_edges_type2id_to_num[now_two_pre_id].find(now_two_edges_entity_type_id) != two_edges_type2id_to_num[now_two_pre_id].end()){
//                                            如果有这样的 two_pre_id
...
//                                            cout << kv_store->getEntityByID(now_two_edges_entity_type_id.s1_type_id) << "  " << kv_store->getPredicateByID(pre_id) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << "  " << kv_store->getPredicateByID(po_list[j]) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << endl;
                                            ...
                                            two_edges_type2id_to_num[now_two_pre_id].insert(make_pair(now_two_edges_entity_type_id, 1));
                                            vector<TWO_EDGES_ENTITY_TYPE_ID> sample_vector;
                                            sample_vector.push_back(now_two_edges_entity_id);
...
//            找到 (s1)-[p1]->(s2)
            if(entityid_to_typeid_map.count(p1_so_id_list[i]) > 0 &&
               entityid_to_typeid_map.count(p1_so_id_list[i+1]) > 0){
                unsigned* po_list;
                unsigned list_len = 0;
                kv_store->getpreIDsubIDlistByobjID(p1_so_id_list[i+1], po_list, list_len, true);
                for(unsigned j = 0; j < list_len; j += 2){
//                    找到有 type 的 (s3)
                    if(entityid_to_typeid_map.count(po_list[j+1]) > 0){
//                        这是这个函数的主体
                        ...
                        for(auto s1 = s1_range.first; s1 != s1_range.second; ++s1){
                            for(auto s2 = s2_range.first; s2 != s2_range.second; ++s2){
                                for(auto s3 = s3_range.first; s3 != s3_range.second; ++s3){
                                    ...
                                    if(two_edges_type3id_to_num.find(now_two_pre_id) == two_edges_type3id_to_num.end()){
...
//                                        cout << kv_store->getEntityByID(now_two_edges_entity_type_id.s1_type_id) << "  " << kv_store->getPredicateByID(pre_id) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << "  " << kv_store->getPredicateByID(po_list[j]) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << endl;
                                        ...
                                        unordered_map<TWO_EDGES_ENTITY_TYPE_ID, int, TWO_EDGES_ENTITY_TYPE_ID::TWO_EDGES_ENTITY_TYPE_ID_Hash> this_type_num;
                                        this_type_num.insert(make_pair(now_two_edges_entity_type_id, 1));
                                        two_edges_type3id_to_num.insert(make_pair(now_two_pre_id, this_type_num));
                                        ...
                                        if(two_edges_type3id_to_num[now_two_pre_id].find(now_two_edges_entity_type_id) != two_edges_type3id_to_num[now_two_pre_id].end()){
//                                            如果有这样的 two_pre_id
...
//                                            cout << kv_store->getEntityByID(now_two_edges_entity_type_id.s1_type_id) << "  " << kv_store->getPredicateByID(pre_id) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << "  " << kv_store->getPredicateByID(po_list[j]) << "  " << kv_store->getEntityByID(now_two_edges_entity_type_id.s2_type_id) << endl;
                                            ...
                                            two_edges_type3id_to_num[now_two_pre_id].insert(make_pair(now_two_edges_entity_type_id, 1));
                                            vector<TWO_EDGES_ENTITY_TYPE_ID> sample_vector;
                                            sample_vector.push_back(now_two_edges_entity_id);
...
//    pre1_id, pre1_edge_n,
//    s1_type_id,  o1_type_id, s1_p1_o1_num,
...
//    注意: 只存了抽样结果表, 该条抽样归于哪一类可以有 s1_s2_s3_p1_p2_num 来判断
...
//    p1_p2_s1_s2_s3_s4id, p1_p2_s1_s2_s3_s5id, p1_p2_s1_s2_s3_s6id,...
    ...
    for(auto x = two_edges_type2id_to_num.begin(); x != two_edges_type1id_to_num.end(); ++x ){
        fwrite(&x->first.p1_id, sizeof(TYPE_PREDICATE_ID), 1, num_file);
        fwrite(&x->first.p2_id, sizeof(TYPE_PREDICATE_ID), 1, num_file);
        int size = (int)x->second.size();
        fwrite(&size, sizeof(int), 1, num_file);
...
//    p1_p2_s1_s2_s3_s4id, p1_p2_s1_s2_s3_s5id, p1_p2_s1_s2_s3_s6id,...
    ...
    for(auto x = two_edges_type3id_to_num.begin(); x != two_edges_type1id_to_num.end(); ++x ){
...
//            s_type_id, o_type_id
            ...
            int this_type_sample_num = min(this_type_num, SAMPLE_NUM_UPBOUND);

                                                            ...
                                                            bool remain_old)
{
  ...
  stringstream ss;
  ss << "JoinTable public_variables_ ";
 ...
 */
void gstore::Executor::UpdateIDList(const shared_ptr<IDList> &valid_id_list, unsigned *id_list, unsigned id_list_len, bool id_list_prepared)
{
  if (id_list_prepared)
  {
                                                                            ...
                                                                            bool remain_old)
{
  ...
  if (edge_table_info.ConstantToVar(edge_info))
  {
    ...
    {
    case JoinMethod::s2p:
    { // Because if we don't add a pair of '{}', the editor will report a error of redefinition
...
bool gstore::Executor::AddConstantCandidates(EdgeInfo edge_info, TYPE_ENTITY_LITERAL_ID targetID, bool distinct, IDCachesSharePtr id_caches)
{
  ...
  default:
  {
    ...
    // assert(false);
    throw runtime_error("Error in Executor::AddConstantCandidates, unknown join method");
  }
  }
  auto validate_list = make_shared<vector<TYPE_ENTITY_LITERAL_ID>>(edge_candidate_list, edge_candidate_list + this_edge_list_len);
...
shared_ptr<IDList> gstore::Executor::CandidatesWithConstantEdge(shared_ptr<vector<EdgeInfo>> edge_info_vector, bool distinct) const
{
  ...
  for (unsigned i = 0; i < edge_info_vector->size(); i++)
  {
    ...
    switch (edge_info.join_method_)
    {
      ...
      default:
      {
        SLOG_CORE("Error in Executor::CandidatesWithConstantEdge, unknown join method");
        // assert(false);
        throw runtime_error("Error in Executor::CandidatesWithConstantEdge, unknown join method");
      }
    }

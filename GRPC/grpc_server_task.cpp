...
GRPCServerTask::GRPCServerTask(CommService *service,
                               ProcFunc& process) :
        WFServerTask(service, WFGlobal::get_scheduler(), process),
        req_is_alive_(false),
        req_has_keep_alive_header_(false)
{
    WFServerTask::set_callback([this](GRPCTask *task) {
...
void GRPCServerTask::handle(int state, int error)
{
...
CommMessageOut *GRPCServerTask::message_out()
{
    GRPCResp *resp = this->get_resp();

    std::map<std::string, std::string, MapStringCaseLess> &headers = resp->headers;
    // content type
    if(headers.find("Content-Type") == headers.end())
    {
        headers["Content-Type"] = ContentType::to_str(TEXT_PLAIN);
    }
...
std::string GRPCServerTask::peer_addr() const
{
...
unsigned short GRPCServerTask::peer_port() const
{
    struct sockaddr_storage addr;
    socklen_t addr_len = sizeof addr;
    this->get_peer_addr(reinterpret_cast<struct sockaddr *>(&addr), &addr_len);

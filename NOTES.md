...
find the max using ID need to loop and call getEntityByID
but if using StringIndex, and if ID is removed(now not reported ad update), a real string will
be returned, which will cause error
(when insert, find the ID, if exist then fill, otherwise push)
---
is query answer not right ->this reason?
I think no, because original gstore_with_insert_delete also reports the same result!
(also not the problem of satellites generating duplicates)
